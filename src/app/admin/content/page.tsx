          ...
          {PAGES.map(p => (
            <button key={p.id} onClick={() => setActivePage(p.id)}
              className={`px-4 py-2 rounded-xl text-sm font-semibold transition-colors ${
                activePage === p.id ? 'bg-blue text-white' : 'bg-white text-navy/60 border border-blue/20 hover:border-blue'
              }`}>
              {p.label}
            </button>
          ))}
          ...
          <div className="space-y-5 bg-white rounded-2xl border border-blue/10 p-6 shadow-sm">
            {fields.map(f => (
              <div key={f.key}>
                ...
                {f.type === 'textarea' ? (
                  <textarea rows={8} className="form-input resize-y font-mono text-xs"
                    value={content[f.key] || ''}
