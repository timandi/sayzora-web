    ...
    <AdminShell>
      <div className="p-8">
        <div className="flex items-center justify-between mb-6">
          ...
          <button onClick={() => setCreating(true)} className="btn btn-primary btn-sm">+ New Apartment</button>
        </div>
        ...
        {loading ? (
          ...
          <div className="bg-white rounded-2xl border border-blue/10 shadow-sm overflow-hidden">
              ...
              <thead className="bg-slate-50 border-b border-blue/10">
                <tr>
                  {['#','Name','Location','BR','BA','Status','Actions'].map(h => (
                    <th key={h} className="px-4 py-3 text-left text-navy/60 font-semibold text-xs uppercase tracking-wide">{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
                {apts.map((apt, i) => (
                  <tr key={apt.id} className="border-b border-slate-100 hover:bg-slate-50">
                    ...
                    <td className="px-4 py-3">
                      <button onClick={() => toggleActive(apt)}
                        className={`px-2 py-0.5 rounded-full text-xs font-semibold ${apt.active ? 'bg-green-100 text-green-700' : 'bg-slate-100 text-slate-500'}`}>
                        {apt.active ? 'Active' : 'Hidden'}
                      </button>
                    </td>
                    <td className="px-4 py-3 flex gap-2">
                      <button onClick={() => setEditing(apt)} className="btn btn-ghost btn-sm text-xs border border-blue/20">Edit</button>
                      <button onClick={() => deleteApt(apt.id)} className="btn btn-danger btn-sm text-xs">Delete</button>
                    </td>
                  </tr>
                ))}
              </tbody>
            ...
            {apts.length === 0 && (
              <p className="text-center text-navy/40 py-8">No apartments yet. Add your first one!</p>
            )}
          </div>
        )}
      </div>
