...
export async function GET(req: NextRequest, { params }: { params: { id: string } }) {
  ...
  const db = getDb();
  const row = db.prepare('SELECT * FROM apartments WHERE id = ?').get(params.id);
...
export async function PUT(req: NextRequest, { params }: { params: { id: string } }) {
  ...
  const existing = db.prepare('SELECT * FROM apartments WHERE id = ?').get(params.id);
  ...
  return NextResponse.json(db.prepare('SELECT * FROM apartments WHERE id = ?').get(params.id));
}

export async function DELETE(req: NextRequest, { params }: { params: { id: string } }) {
  const err = await auth(req); if (err) return err;
  const db = getDb();
  db.prepare('DELETE FROM apartments WHERE id = ?').run(params.id);
