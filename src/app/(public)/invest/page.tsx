  ...
  return (
    <>
      ...
      <div className="bg-gradient-to-b from-navy to-ocean pt-32 pb-20 text-center px-4">
        ...
        <p className="text-white/65 text-lg max-w-2xl mx-auto">Tenerife's year-round tourism economy offers strong yields and high occupancy. We own nothing — you do. We manage everything.</p>
      </div>
          ...
          <div className="grid md:grid-cols-4 gap-6">
            {steps.map((s) => (
              <div key={s.n} className="text-center">
                <div className="w-14 h-14 rounded-full bg-blue/10 text-blue font-bold text-lg flex items-center justify-center mx-auto mb-4">{s.n}</div>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <input name="first_name" type="text" required placeholder="Your name" className="form-input bg-white/10 border-white/20 text-white placeholder:text-white/40 focus:border-gold" />
              </div>
              <div>
                <label className="block text-white/80 text-sm font-semibold mb-1.5">Phone</label>
                <input name="last_name" type="tel" placeholder="+34 …" className="form-input bg-white/10 border-white/20 text-white placeholder:text-white/40 focus:border-gold" />
              </div>
            </div>
            <div>
              <label className="block text-white/80 text-sm font-semibold mb-1.5">Email</label>
              <input name="email" type="email" required placeholder="you@example.com" className="form-input bg-white/10 border-white/20 text-white placeholder:text-white/40 focus:border-gold" />
            </div>
            <div>
              <label className="block text-white/80 text-sm font-semibold mb-1.5">Investment Range</label>
              <select name="message" className="form-input bg-white/10 border-white/20 text-white focus:border-gold">
                <option value="Under €150,000" className="text-navy">
                  Under €150,000
                </option>
                <option value="€150,000 – €250,000" className="text-navy">
                  €150,000 – €250,000
                </option>
                <option value="€250,000 – €500,000" className="text-navy">
                  €250,000 – €500,000
                </option>
                <option value="€500,000+" className="text-navy">
                  €500,000+
                </option>
              </select>
            </div>
            <button type="submit" disabled={status === "loading"} className={`btn btn-gold btn-md w-full ${status === "loading" ? "opacity-70" : ""}`}>
