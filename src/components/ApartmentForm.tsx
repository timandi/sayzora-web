...
const AMENITY_OPTIONS = [
  'Sea Views','Beachfront','Pool','Outdoor Pool','Private Pool','Air Conditioning',
  'WiFi','Balcony','Terrace','Garden','Mountain View','Teide View','Atlantic View',
  'Walk-in Shower','Washing Machine','Satellite TV','Streaming TV','Parking',
  'Disabled Access','Kids Pool','Private Beach Area','Private Entrance','Bidet',
];
    ...
    <form onSubmit={handleSubmit} className="space-y-6 bg-white rounded-2xl border border-blue/10 p-6 shadow-sm">
          ...
          {AMENITY_OPTIONS.map(a => (
            <button type="button" key={a} onClick={() => toggleAmenity(a)}
              className={`px-3 py-1 rounded-full text-xs font-medium border transition-colors ${
                form.amenities.includes(a) ? 'bg-blue text-white border-blue' : 'bg-white text-navy/60 border-blue/20 hover:border-blue'
              }`}>
              {a}
            </button>
          ))}
      ...
      <div>
        ...
        <div className="flex gap-2 mt-1">
          ...
          <button type="button" onClick={addPhotoUrl} className="btn btn-ghost btn-sm border border-blue/20">Add URL</button>
        </div>
            ...
            {form.photos.map(p => (
              <div key={p} className="relative group aspect-square rounded-lg overflow-hidden bg-slate-100">
                ...
                <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center gap-1">
                  <button type="button" onClick={() => setForm(f => ({ ...f, cover_photo: p }))}
                    className={`text-xs px-1.5 py-0.5 rounded ${form.cover_photo === p ? 'bg-gold text-navy' : 'bg-white/80 text-navy'}`}>
                    {form.cover_photo === p ? '★ Cover' : 'Cover'}
                  </button>
                  <button type="button" onClick={() => removePhoto(p)} className="text-xs bg-red-500 text-white px-1.5 py-0.5 rounded">✕</button>
                </div>
              </div>
            ))}
          </div>
